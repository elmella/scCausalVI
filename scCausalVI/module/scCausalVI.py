...
#         self.gammas = gammas
#         self.fused_layer_norm = torch.nn.LayerNorm(n_hidden)
...
#     @torch.jit.script
#     def _compute_local_library_params(self, batch_index: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
#         n_batch = self.library_log_means.shape[1]
#         local_library_log_means = F.linear(
#             one_hot(batch_index, n_batch), self.library_log_means, out=batch_index.new_zeros(batch_index.size(0), 1)
#         )
#         local_library_log_vars = F.linear(
#             one_hot(batch_index, n_batch), self.library_log_vars, out=batch_index.new_zeros(batch_index.size(0), 1)
#         )
#         return local_library_log_means, local_library_log_vars
...
class scCausalVIModule(BaseModuleClass):
    def __init__(
        ...
        dropout_rate: float = 0.1,
        use_observed_lib_size: bool = True,
        library_log_means: torch.Tensor = None,
        library_log_vars: torch.Tensor = None,
        use_mmd: bool = True,
        mmd_weight: float = 1.0,
        norm_weight: float = 1.0,
        ...
        if not self.use_observed_lib_size:
            self.register_buffer("library_log_means", library_log_means)
            self.register_buffer("library_log_vars", library_log_vars)
    ...
    def _compute_local_library_params(self, batch_index: torch.Tensor):
        n_batch = self.library_log_means.shape[1]
        local_library_log_means = F.linear(
            F.one_hot(batch_index, n_batch).float(), self.library_log_means
        )
        local_library_log_vars = F.linear(
            F.one_hot(batch_index, n_batch).float(), self.library_log_vars
        )
        return local_library_log_means, local_library_log_vars

    def _generic_inference(self, x, batch_index, condition_label, src):
        x_ = torch.log(1 + x)
        n_cells = x.shape[0]
        ...
        if src == 'control':
            ...
            for t_lbl in unique_treats:
                ...
                treat_name = next(k for k, v in self.condition2int.items() if v == t_lbl.item())
                mask = (condition_label == t_lbl).squeeze()
                x_sub = x_[mask]
                batch_sub = batch_index[mask]
    ...
    def inference(self, x, condition_label, batch_index, n_samples=1):
        ctrl_mask = (condition_label == self.condition2int[self.control]).squeeze()
        ...
        inference_control = self._generic_inference(
            x_control, batch_control, condition_label=condition_control, src='control'
        )
        inference_treatment = self._generic_inference(
            x_treatment, batch_treatment, condition_label=condition_treatment, src='treatment'
        )

        return {"control": inference_control, "treatment": inference_treatment}

    def generative(self, z_bg, z_t, library, batch_index):
        attention_weights = torch.softmax(self.attention(z_t), dim=-1)
        z_t = attention_weights * z_t
        latent = torch.cat([z_bg, z_t], dim=-1)
        px_scale, px_r, px_rate, px_dropout = self.decoder(
    ...
    def forward(self, *args, **kwargs):
        tensors = self._get_inference_input(*args, **kwargs)
        inference_outputs = self.inference(**tensors)
    ...
    def loss(
        ...
        px_rate, px_r, px_dropout = generative_outputs['px_rate'], generative_outputs['px_r'], generative_outputs['px_dropout']

        px_rate_ctrl, px_dropout_ctrl = px_rate[ctrl_mask], px_dropout[ctrl_mask]
        px_rate_trt, px_dropout_trt = px_rate[~ctrl_mask], px_dropout[~ctrl_mask]

        elbo_ctrl = self._generic_loss(
            x_ctrl, batch_ctrl, ctrl_inference['qbg_m'], ctrl_inference['qbg_v'],
            ctrl_inference['library'], ctrl_inference['ql_m'], ctrl_inference['ql_v'],
            px_rate_ctrl, px_r, px_dropout_ctrl,
        )

        elbo_trt = self._generic_loss(
            x_trt, batch_trt, trt_inference['qbg_m'], trt_inference['qbg_v'],
            trt_inference['library'], trt_inference['ql_m'], trt_inference['ql_v'],
            px_rate_trt, px_r, px_dropout_trt,
        )
        ...
        if self.use_mmd:
            loss_mmd = self._compute_mmd_loss(ctrl_inference["z_bg"], trt_inference["z_bg"], condition_label[~ctrl_mask])

        loss_norm = torch.tensor(0.0, device=x.device)
        if self.norm_weight > 0:
            loss_norm = self._compute_norm_loss(trt_inference['z_t'])
