    ...
    def setup_anndata(
        ...
        **kwargs,
        ...
        anndata_fields = [
            ...
            CategoricalObsField(SCCAUSALVI_REGISTRY_KEYS.BATCH_KEY, batch_key),
            CategoricalObsField(SCCAUSALVI_REGISTRY_KEYS.CONDITION_KEY, condition_key),
            NumericalObsField(SCCAUSALVI_REGISTRY_KEYS.SIZE_FACTOR_KEY, size_factor_key, required=False),
            CategoricalJointObsField(SCCAUSALVI_REGISTRY_KEYS.CAT_COVS_KEY, categorical_covariate_keys),
            NumericalJointObsField(SCCAUSALVI_REGISTRY_KEYS.CONT_COVS_KEY, continuous_covariate_keys),
        ]
        ...
        for tensors in data_loader:
            ...
            for lbl in unique_labels:
                ...
                if lbl.item() == control_label_idx:
                    ...
                    chosen_bg = outputs["qbg_m"] if give_mean else outputs["z_bg"]
                    bg_holder[mask] = chosen_bg.detach().cpu()
                else:
                    treat_name = label_to_name.get(lbl.item(), None)
                    if treat_name is None:
                ...
                if lbl.item() == control_label_idx:
                    ...
                    if source_label_idx == control_label_idx:
                        if target_label_idx != control_label_idx:
                            ...
                            if target_name is not None:
                                s_enc = self.module.treatment_te_encoders[target_name]
                                tm, tv, zt = s_enc(outputs["z_bg"])
                                chosen_t = tm if give_mean else zt
                                t_holder[mask] = chosen_t.detach().cpu()
                            else:
                                raise ValueError(f"Unknown treatment: {target_name}")
                        else:
                            raise ValueError("target_condition should be different from source_condition.")
                else:
                    tname = label_to_name.get(lbl.item(), None)
                    if tname is None:
                        raise ValueError(f"Unknown treatment: {tname} in model.")
        ...
        for tensors in data_loader:
            ...
            def get_count_expression_cross_condition(
                ...
                for tensors in data_loader:
                    ...
                    latent_bg_tensor = torch.zeros([x.shape, self.module.n_background_latent], device=self.module.device)
                    latent_t_tensor = torch.zeros([x.shape, self.module.n_te_latent], device=self.module.device)
                    latent_library_tensor = torch.zeros([x.shape, 1], device=self.module.device)
                    predicted_label = torch.zeros([x.shape, 1], device=self.module.device)
                        ...
                        if lbl.item() == control_label_idx:
                            ...
                            if source_label_idx == control_label_idx:
                                if target_label_idx != control_label_idx:
                                    target_name = label_to_name.get(target_label_idx, None)
                                    if target_name is not None:
                                        tm, tv, zt = self.module.treatment_te_encoders[target_name](outputs["z_bg"])
                                        attn = torch.softmax(self.module.attention(zt), dim=-1)
                                        zt = attn * zt
                                        latent_t_tensor[mask] = zt
                                        predicted_label[mask] = target_label_idx
                                    ...
                                    raise ValueError("target condition should be different from source condition.")
                            else:
                                predicted_label[mask] = label_index[mask]
                        else:
                            tname = label_to_name.get(lbl.item(), None)
